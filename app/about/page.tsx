...
// GROQ Query — sab staff fetch karega
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 pt-20">
      ...
      <section className="py-16 px-4 sm:px-6 bg-gradient-to-r from-cyan-500/10 via-blue-500/10 to-purple-500/10">
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            <div>
              <div className="text-4xl font-bold text-cyan-600 dark:text-cyan-400">10K+</div>
            ...
            <div>
              ...
              <div className="text-sm text-slate-600 dark:text-slate-400">Expert Mentors</div>
            </div>
            <div>
              <div className="text-4xl font-bold text-indigo-600 dark:text-indigo-400">2026</div>
      ...
      <section className="py-16 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-3xl font-bold text-slate-800 dark:text-white text-center mb-12">
            Meet Our <span className="bg-gradient-to-r from-cyan-500 to-purple-600 bg-clip-text text-transparent">Team</span>
          </h2>

          {staffMembers.length === 0 ? (
            <p className="text-center text-slate-600 dark:text-slate-400">No team members added yet. Please add some in Sanity.</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {staffMembers.map((staff: any) => (
                ...
                >
                  <div className="w-20 h-20 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-full flex items-center justify-center mx-auto mb-4 text-white text-2xl font-bold">
                    {staff.name?.charAt(0) || '?'}
                  </div>
                  <h3 className="text-xl font-bold text-slate-800 dark:text-white">{staff.name}</h3>
                  <p className="text-cyan-600 dark:text-cyan-400 font-medium">{staff.role}</p>
                  <p className="text-sm text-slate-600 dark:text-slate-400 mt-2">{staff.qualification}</p>
                  {staff.subjects?.length > 0 && (
                    <div className="mt-3 flex flex-wrap justify-center gap-1">
                      {staff.subjects.map((sub: any) => (
                        <span key={sub._id} className="text-xs bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded">
                          {sub.title}
                        </span>
                      ))}
                    </div>
      ...
      <section className="py-16 px-4 sm:px-6 bg-gradient-to-r from-cyan-500/10 via-blue-500/10 to-purple-500/10">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-3xl font-bold text-slate-800 dark:text-white text-center mb-12">
            Why Choose <span className="bg-gradient-to-r from-cyan-500 to-purple-600 bg-clip-text text-transparent">ChemLab Academy?</span>
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="flex gap-4 p-6 bg-white dark:bg-slate-800 rounded-xl border border-slate-200/50 dark:border-slate-700/50">
              ...
              <div>
                <h4 className="font-bold text-slate-800 dark:text-white">Expert Faculty</h4>
                <p className="text-sm text-slate-600 dark:text-slate-400">Learn from PhDs and industry professionals with years of teaching experience.</p>
              </div>
            </div>
            <div className="flex gap-4 p-6 bg-white dark:bg-slate-800 rounded-xl border border-slate-200/50 dark:border-slate-700/50">
              <BookOpen className="h-8 w-8 text-blue-500 flex-shrink-0" />
              <div>
                <h4 className="font-bold text-slate-800 dark:text-white">Comprehensive Curriculum</h4>
                <p className="text-sm text-slate-600 dark:text-slate-400">Structured courses covering O/A-Levels, FSc, and University-level chemistry.</p>
              </div>
            </div>
            <div className="flex gap-4 p-6 bg-white dark:bg-slate-800 rounded-xl border border-slate-200/50 dark:border-slate-700/50">
              <Atom className="h-8 w-8 text-purple-500 flex-shrink-0" />
              <div>
                <h4 className="font-bold text-slate-800 dark:text-white">Interactive Learning</h4>
                <p className="text-sm text-slate-600 dark:text-slate-400">3D molecular visualizations, virtual labs, and interactive simulations.</p>
              </div>
            </div>
            <div className="flex gap-4 p-6 bg-white dark:bg-slate-800 rounded-xl border border-slate-200/50 dark:border-slate-700/50">
              <Users className="h-8 w-8 text-indigo-500 flex-shrink-0" />
              <div>
                <h4 className="font-bold text-slate-800 dark:text-white">Community Support</h4>
                <p className="text-sm text-slate-600 dark:text-slate-400">24/7 doubt support, discussion forums, and peer-to-peer learning.</p>
              </div>
            </div>
          </div>
        </div>
